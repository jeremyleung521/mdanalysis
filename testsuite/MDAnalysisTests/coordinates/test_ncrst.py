...
class TestReadACEBox(_NCRestartTest):
    ...
    def test_positions_resid1(self, universe):
        ...
        expected = np.array(
            [
                [15.24987316, 12.57817841, 15.19173145],
    ...
    def test_positions_resid42(self, universe):
        ...
        expected = np.array(
            [
                [21.78003883, 21.15958595, 19.864048],
    ...
    def create_ncrst(self, params):
        ...
        with netcdf_file(
            ...
            if params["ConventionVersion"]:
                setattr(
                    ncrst, "ConventionVersion", params["ConventionVersion"]
                )
            if params["program"]:
            ...
            if params["scale_factor"]:
                setattr(
                    ncrst.variables[params["scale_factor"]],
                    "scale_factor",
                    2.0,
                )
...
class TestNCRSTReaderExceptionsWarnings(_NCRSTGenerator):
    ...
    @pytest.mark.parametrize(
        "key,value",
        (("version_byte", 1), ("Conventions", "Foo"), ("spatial", 2)),
    )
    ...
    @pytest.mark.parametrize(
        "key,value", (("spatial", None), ("n_atoms", None), ("coords", None))
    )
    ...
    @pytest.mark.parametrize(
        "key,value", (("time", "femtosecond"), ("coords", "nanometer"))
    )
    ...
    @pytest.mark.parametrize(
        "key,value", (("program", None), ("programVersion", None))
    )
    ...
    def test_notime_warn(self, tmpdir):
        ...
        with tmpdir.as_cwd():
            ...
            assert len(record) == 1
            wmsg1 = (
                "NCDF restart file {0} does not contain `time` information;"
